...
const BrowserView: React.FC<BrowserViewProps> = ({ tabId, isHome = false, homePreferences, onOpenShortcut }) => {
  ...
  const isImage = /^(https?:\/\/|data:image\/)/i.test(background.trim());
  const style = isImage ? { backgroundImage: `linear-gradient(rgba(8,9,12,.56), rgba(8,9,12,.88)), url("${background.replace(/"/g, '%22')}")`, backgroundSize: 'cover', backgroundPosition: 'center' } : { background: background.trim() || '#08090c' };
