...
export class BrowserAgentController {
  async run(action: BrowserAgentAction): Promise<BrowserAgentResult> {
    ...
    if (action.type === 'scroll') {
      ...
      await view.webContents.executeJavaScript(`window.scrollBy({ top: ${amount}, behavior: 'smooth' })`, true);
      return { ok: true, action: action.type, message: 'Scrolled the active page.' };
    }
